...
// *** get gauss point weight, test function and test function partial derivatives ***
      ...
      for (unsigned i = 0; i < nDofs; i++) {
        ...
        adept::adouble M_wsxyd =  2. * phi[i] * solwsxydGauss;
    ...
    adept::adouble Bwyyu = 0.;
        ...
        Bwxxu += phi_x[i * dim] * solwGauss_x[0] ;

        Bwxyu +=  ( phi_x[i * dim] * solwGauss_x[1] + phi_x[i * dim + 1 ] * solwGauss_x[0] );

        Bwyyu += phi_x[i * dim + 1] * solwGauss_x[1];

        Bwxxud += phi_x[i * dim] * solwsxxdGauss_x[0] ;

        Bwxyud +=  ( phi_x[i * dim] * solwsxydGauss_x[1] + phi_x[i * dim + 1 ] * solwsxydGauss_x[0] );

        Bwyyud += phi_x[i * dim + 1] * solwsyydGauss_x[1];


        // ----- target u_D and source f for the state PDE -----
        // u_D term = (u_D, phi_i) where u_D = u + w - Delta²ud
        // The MINUS sign is critical for the adjoint PDE RHS!
...
f_u_val = (-72.*xg4 - 96.*xg3 - 864.*xg2*yg2 + 108.*xg2 - 576.*xg*yg2 + 120.*xg - 72.*yg4 + 108.*yg2 + 9.) * phi[i] ;
        ...
        //     In residual form: (∇φ, div λ_σ) + (u + w - u_D, φ) = 0
        //     Expanded: (Bxxd + Bxyd + Byyd) + M_u + M_w - udr_term = 0
        //     where udr_term = u_D * φ and u_D = u + w - Δ²ud
